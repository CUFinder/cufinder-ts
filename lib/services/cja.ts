...
import { CjaParams, CjaResponse } from '../shared/types';

/**
 * CJA - Company Jobs API (V2)
 */
export class CjaService extends BaseService {
    /**
     * Search for company jobs
     * @param params - Optional search parameters
     */
    public async searchCompanyJobs(params: CjaParams = {}): Promise<CjaResponse> {
        try {
            ...
            const response = await this.client.post('/cja', searchParams);

            return this.parseResponseData<CjaResponse>(response.data);
        } catch (error) {
            throw this.handleError(error, 'CJA Service');
        }
    }
}
